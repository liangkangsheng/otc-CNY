			...
			<view class="order-box height" v-if="orderList.length > 0">
				<view class="asseats-lists" v-for="(item, index) in orderList" @click="orderListButton(item)">
					<view class="list-header">
						<text :class="tabIndex == 0 ? 'user-pay' : 'user-pays'">{{ tabIndex == 0 ? i18n.text2 : i18n.text02 }}</text>
						<uni-icons type="arrowright" size="16" class="form-clear-icon arrowright"></uni-icons>
						<text class="title-pay user-pay" v-if="item.status == 1">{{ i18n.text4 }}</text>
						<text class="title-pay user-pay"  v-if="item.status == 2">{{ i18n.text5 }}</text>
						<text class="title-pay user-pay"  v-if="item.status == 3">{{ i18n.text6 }}</text>
						<text class="title-pay user-pays" v-if="item.status == 4">{{ i18n.text7 }}</text>
					</view>
					<view class="list-bottom height">
						...
						<view class="list-left left-middle">
							...
							<view class="size-bottom" >{{ tabIndex == 0?Number(Number(item.buyCount).toFixed(2)): Number(Number(item.saleCount).toFixed(2))}}/{{Number(Number(item.price).toFixed(2)) }}</view>
						</view>
						<view class="list-left list-right">
							<view class="size-top">{{ i18n.text11 }}(CNY)</view>
							<view class="size-bottom">{{ tabIndex == 0?item.buyAmount: item.saleAmount}}</view>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		async getOrderListFuntion() {
			...
			let res = await api.getOrderListsHttp({
		...
		orderListButton(item) {
			if(this.tabIndex == 0){
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/upOrder/cancelledDetails?orderId=' + item.buyInfoId + '&type=' + (Number(this.tabIndex) + 1).toString()
					});
				}, 1000);
			}else{
				setTimeout(() => {
					uni.navigateTo({
						url: '/pages/upOrder/cancelledDetails?orderId=' + item.saleDetailId  + '&status=' + status + '&type=' + (Number(this.tabIndex) + 1).toString()
					});
				}, 1000);
			}
