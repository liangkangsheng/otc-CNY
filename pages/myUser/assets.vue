			...
			<view class="asseats-box height order-box" v-if="WalletLogsData.length > 0">
				<view class="asseats-lists" v-for="(item, index) in WalletLogsData" :key="index">
					<view class="list-header">
						<text :class="item.inOut == 1 ? 'user-pay' : 'user-pays'">{{ item.inOut == 1 ? i18n.text14 : i18n.text15 }}</text>
						<uni-icons type="arrowright" size="16" class="form-clear-icon arrowright"></uni-icons>
						<text class="title-pay" :class="item.inOut == 1 ? 'user-pay' : 'user-pays'" @click="AssetsButton(index)">{{ item.inOut == 1 ? i18n.text14 : i18n.text15 }}</text>
						<text class="status-btns" :class="item.inOut == 1 ? 'user-pay-back' : 'user-pays-back'"  @click="AssetsButton(index)"></text>
					</view>
					<view class="list-bottom height">
						<view class="list-left left-middle">
							<view class="size-top">{{ i18n.text1 }}</view>
							<view class="size-bottom">
								<span v-if="item.typeId == 2">{{i18n.text2}}</span>
								<span v-else-if="item.typeId == 3">{{i18n.text3}}</span>
								<span v-else-if="item.typeId == 4">{{i18n.text4}}</span>
								<span v-else-if="item.typeId == 5">{{i18n.text5}}</span>
								<span v-else-if="item.typeId == 9">{{i18n.text21}}</span>
								<span v-else-if="item.typeId == 12">{{i18n.text22}}</span>
								<span v-else-if="item.typeId == 13">{{i18n.text23}}</span>
								<span v-else-if="item.typeId == 14">{{i18n.text6}}</span>
								<span v-else-if="item.typeId == 16">{{i18n.text20}}</span>
								<span v-else-if="item.typeId == 25">{{i18n.text24}}</span>
								<span v-else-if="item.typeId == 30">{{i18n.text7}}</span>
								<span v-else-if="item.typeId == 31">{{i18n.text8}}</span>
								<span v-else-if="item.typeId == 32">{{i18n.text18}}</span>
								<span v-else-if="item.typeId == 33">{{i18n.text17}}</span>
								<span v-else-if="item.typeId == 40">{{i18n.text9}}</span>
								<span v-else-if="item.typeId == 41">{{i18n.text19}}</span>
								<span v-else-if="item.typeId == 42">{{i18n.text019}}</span>
								<span v-else>{{i18n.text16}}</span>
							</view>
						</view>
						...
						<view class="list-left list-right">
							<view class="size-top">{{ i18n.text11 }}({{item.coinSymbol}})</view>
							<view class="size-bottom" :class="item.inOut == 1 ? 'user-pay' : 'user-pays'">{{ (item.inOut == 1 ? '+' : '-') + item.cost }}</view>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		async getMyAssetsItemFunction() {
			uni.showLoading({ title: this.$t('assets.text13'), mask: true });
			...
			if (res.code === '000') {
				...
				uni.hideLoading();
				this.$alert(this.$t('assets.text600'))
			}  else {
				uni.hideLoading();
		...
		async queryWalletLogsFunction() {
			...
			let res = await api.queryWalletLogsHttp({
				coinSymbol: 'USDT',
