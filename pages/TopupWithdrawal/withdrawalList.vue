				...
				<view class="asseats-lists" v-for="(item, index) in WalletLogsData" :key="index">
					<view class="list-bottom height">
						<view class="list-left left-middle">
							...
							<view class="size-bottom">{{ item.money }}</view>
						</view>
						...
						<view class="list-left list-right" @click="AssetsButton(item.id)">
							...
							<view class="size-bottom">
								<span v-if="item.status == 1" style="color: blue;">
									{{ i18n.text2 }}
									<uni-icons
										style="color: blue;"
										@click="AssetsButton(item.id)"
										type="arrowright"
										size="16"
										class="form-clear-icon arrowright pull-right"
									></uni-icons>
								</span>
								<span v-else-if="item.status == 5" style="color: #de6334;">
									{{ i18n.text3 }}
									<uni-icons
										style="color: #de6334;"
										type="arrowright"
								...
								<span v-else-if="item.status == 10" style="color: #de6334;">
									{{ i18n.text4 }}
									<uni-icons
										style="color: #de6334;"
										type="arrowright"
								...
								<span v-else-if="item.status == 15" style="color: #06b572;">
									{{ i18n.text5 }}
									<uni-icons
										style="color:  #06b572;"
										type="arrowright"
								...
								<span v-else-if="item.status == 20" style="color: #de6334;">
									{{ i18n.text6 }}
									<uni-icons
										style="color: #de6334;"
										type="arrowright"
...
export default {
	...
	onLoad() {
		uni.setNavigationBarTitle({
			title: this.$t('withdrawalList.text12')
		});
	...
	methods: {
		...
		AssetsButton(id) {
			uni.navigateTo({
				url: '/pages/TopupWithdrawal/withdrawalDateils?id=' + id
			});
		},
		onDayClick(data) {
		...
		async getMyAssetsItemFunction() {
			uni.showLoading({ title: this.$t('withdrawalList.text13'), mask: true });
			...
			if (res.code === '000') {
				...
				uni.hideLoading();
				this.$alert(this.$t('withdrawalList.text600'))
			} else {
				uni.hideLoading();
		...
		async queryWalletLogsFunction() {
			...
			let res = await api.queryWalletCashHttp({
